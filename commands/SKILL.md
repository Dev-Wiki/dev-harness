...
- 仓库不可访问、整体证据被截断或没有任何可识别的验证入口时，停止并报告全局缺口。
- 某条命令需要私有环境但无法验证时，仅将该记录保留为 `candidate` 并说明前置条件，不影响其他已确认记录。
- 无法判断 quick 与 full 的职责边界时，仅保持相应记录未确认；先保存可确认的 build/test/bugfix。
- 没有独立构建步骤时可将 build 记为 `N/A`；不存在的验证入口为 `Missing`。缺少构建不等于缺少所有验证能力。

单条记录的缺口不得升级为整个映射任务停止，也不得靠推断把它提升为 `confirmed`。下游根据当前任务实际需要的命令决定是否受阻。
