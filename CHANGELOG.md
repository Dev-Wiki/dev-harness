...
- 版本 zip 改为分发可直接使用的 `skills/`，保留 Skill 所需的运行脚本、模板和参考资料，以及专用安装说明、VERSION 和 CHANGELOG。源码、源码安装器及仓库维护文档由对应版本的源码归档提供，不再重复打入 Skill 包；解压后直接复制 Skill 目录安装。
- 中英文 README 明确区分“源码＋脚本安装”和“dist 包直接复制安装”，补充宿主目录、单 Skill 依赖和升级说明；发布包回归验证完整资源、版本一致性及脱离源码仓库后的运行入口。
...
- 增加 Context 文件安全与证据完整性、Audit 修复重验、Auto Fix 验证失效及提交恢复的回归测试。

### Changed

- Auto Fix 状态升级为 SchemaVersion 3，分离内容指纹与暂存状态，持久化验证依赖绑定和授权提交回执。旧状态撤销旧完成结论并要求重新验证、审查；已提交但没有可靠回执的旧运行需先核对提交，再建立新运行。
- Git Workflow 使用通用 TaskChangedFiles 与任务基线检查提交归属；Context 和 Git Workflow 复用既有 doc/ 或 docs/ 文档根。
- Commands 按记录保留未确认缺口；移动项目按具体设备需求执行可用测试；Audit 仅在用户明确要求时读取 LESSONS.md。

### Fixed

- Auto Fix 将新验证绑定当前内容，并在 final-verify、report 和 commit 重查完成门禁，拒绝过期验证、失败审查及失败验证。已知失败不能通过阶段切换、旧通过记录或 skipped 清除；客观跳过必须记录阻塞证据、替代验证和剩余风险，最终最多为 DONE_WITH_CONCERNS。
- Auto Fix 支持相同内容暂存、合法提交后恢复及提交回执写入前中断恢复；按 Git 有效权限位检查内容，显式权限位变化会使旧审查失效。
- Audit 的 resolved 必须关联真实历史确认记录，保留原问题依据，并提供新快照和通过的修复验证；渲染及完成前重新检查，后续漂移会撤销解决结论。
- Context 指纹覆盖 packages/ 工作区、隐藏工程配置及完整文件内容，拒绝引用快照未覆盖的证据；明确的 Unknown、低置信度和空列表不再被离线 Profile 填成事实。
- Context 刷新在写入前复核预览时的文件内容与身份，拒绝覆盖确认期间的用户修改。
- Planning 快照纳入既有 dirty 文件内容与索引指纹；同一任务重开后再次归档使用独立 closure 编号，并修正验证示例掩盖早期失败的问题。

### Security

- Context 初始化与刷新拒绝符号链接等非普通输出文件，缺失文件采用排他创建，阻止悬空软链接越界写入和创建竞争覆盖。
...
- Audit 调用链要求逐条提供调用、调度或数据传递证据，将初始化等前置条件单独列出，避免把生命周期先后关系误写为连续调用。
- 跨模块复核增加未知入口与调用关系检查；未找到公开接口的上层调用时，明确验证边界，不推断业务自动触发，也不因此否定合法调用条件下已证实的接口缺陷。
...
- Codebase Audit 新增 `batch`、`validate-outputs` 和 `render-output`，支持通过 UTF-8 JSON 文件或 stdin 批量登记证据、校验路径并生成紧凑审计文档。

### Changed

- Audit 新运行优先批量保存 Task、Finding 和文档元数据，自动生成导航与快照引用，减少重复手写 patch；旧单项命令和手写产物保持兼容。
- Audit 状态命令支持摘要返回，同次工作区校验复用 Git 身份检查，减少重复进程调用与上下文开销；跨模块复核仍为必经门禁。

### Fixed

- 结构化 Audit 运行完成前检查当前 Revision 已生成文档且内容未被外部修改；生成器保护既有手写产物，支持按文件原子替换与中断后重试。
...
- `dev-harness-planning` 新增 `🟢 待执行` 状态与执行包完整性门禁，输出可供新对话或外部执行工作流使用的模型无关计划产物，但不负责任务领取或执行编排。
- Planning 任务模板补充权威上下文、代码与测试入口、不变量、已确认决策和停止条件，使新对话无需继承规划聊天历史即可执行单个任务。
