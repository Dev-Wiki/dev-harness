...
对一次已发生的工作进行显式项目复盘。它维护复盘历史和“待纳入正式规范的候选结论”（Promotion Candidates），不是每个任务的自动收尾步骤，也不能把一次 AI 失误静默升级为永久项目规则。

## 输出语言

- 用户明确要求英文时使用英文；否则，中文项目或新建且未指定语言的 `LESSONS.md` 默认使用简体中文。
- 刷新既有文档时跟随其主体语言；只编写或改写本次复盘涉及的文字，不顺带全文翻译。
- 中文标题、表头、结论、证据、适用范围和最终报告使用中国人习惯的自然中文，不生硬套用英文句式。
- 路径、命令、代码符号、API/协议/产品名、必要缩写和 `FACT` / `POLICY` / `LESSON` 分类值保持原样；首次面向读者展示时先说明它们的中文含义。
...
| 类型 | 含义 | 进入条件 | 默认去向 |
|------|------|----------|----------|
| `FACT` | 当前项目可客观验证的事实 | 有仓库路径、配置、测试或运行证据 | `LESSONS.md` 中的候选结论；写入 Context 等正式文档后不再重复维护 |
| `POLICY` | 开发者或团队决定采用的规范 | 有用户明确确认；Git 历史和既有习惯只能作为候选证据 | `LESSONS.md` 中的候选结论，等待写入对应正式规范 |
...
稳定结论应写入现有项目契约（Project Contract）中对应的权威文档，而不是要求所有 Agent 永久加载整份历史：
...
Retro 只输出候选结论。写入正式规范前必须再次检查归属文档的写入边界；`POLICY` 必须取得用户确认。不得在一次 Retro 中静默修改多个 Skill 的正式产物。
