...
description 应说明改动的具体目的。除非用户明确要求英文，否则默认使用自然中文，例如 `feat(planning): 拆分活跃任务并归档历史计划`；不使用 “fix bug” 或 “update code” 等笼统描述。

提交前必须检查完整工作区和暂存区。已有暂存内容时仅提交已暂存文件；发现疑似密钥、`.env`、意外大文件、无关改动或临时调试输出时，停止提交并请用户确认。
