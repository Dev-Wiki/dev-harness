...
仓库 [`VERSION`](../VERSION) 与 [`CHANGELOG.md`](../CHANGELOG.md) 最新版本条目均为 `v1.10.0`。每次修改清单后必须根据下表重新统计；“待确认”不计入总数。

| 版本范围 | 已支持 | 部分支持 | 试验性 | 已弃用 |
|---|---:|---:|---:|---:|
| 当前开发版本（v1.10.0） | 30 | 0 | 0 | 0 |
| 最新发布版本（v1.10.0） | 30 | 0 | 0 | 0 |
...
| ID | 功能说明 | 支持状态 | 适用范围 | 版本归属 | 验证方式 | 证据 | 详情 |
|---|---|---|---|---|---|---|---|
| PLN-001 | 从需求、PRD、原型或参考格式生成同一文档根下的 `plan/Dashboard.md` 和 `TaskDetails.md` | 已支持 | 项目、版本或里程碑级规划 | 已发布：v1.10.0 | 契约测试 + 资源安装测试 | `tests/test_docs_contract.py:153-160`；`tests/test_install.py:59-69` | [Planning 契约](../planning/SKILL.md) |
| PLN-002 | 刷新计划时按 Task ID 合并，保留有效 ID、链接、本地状态和有证据的完成状态 | 已支持 | 已有 Dashboard / TaskDetails | 已发布：v1.10.0 | 契约测试 | `tests/test_vnext_contract.py:185-190`；`planning/SKILL.md:67-84` | [Planning 契约](../planning/SKILL.md) |
