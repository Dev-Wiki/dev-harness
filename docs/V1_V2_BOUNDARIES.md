...
设计方案中的 **VNext** 指从既有 V1 向 v1.8.0 Project Contract 与 Codebase Audit 功能的兼容演进，仍属于本文件定义的 V1 范围；v1.9.0 在同一边界内补充 Capability Catalog 与 Audit 文档入口可达性，v1.9.1 进一步统一中文术语和模板表达，v1.10.0 明确 Engineering Audit 职责、输出语言约定、最小行为验证和 Finding identity gate，v1.11.0 增加有界计划分片、旧版单体计划迁移和全 Skill 自然中文输出契约，v1.11.1 再将 Dashboard 收敛为唯一活跃计划入口并增加规划漂移门禁，v1.11.2 为 Auto Fix 增加风险分档验证、结构化证据复用和状态写入快速失败，v1.11.3 为 Planning 增加待执行门禁与可跨对话交接的单任务执行包，v1.11.4 加固安装导出、Context 预览与截断证据、Audit 复核失效和 Auto Fix 最终差异门禁，并补齐离线发布包文档，均不改变 V2 边界。VNext 不是 V2 的别名。当前功能、版本边界和后续候选分别以 `CAPABILITIES.md`、本文件和 `V2_BACKLOG.md` 为准，设计记录中的实施阶段不作为活动任务清单。

## V1 定位

v1.11.8 将版本 zip 收敛为可直接复制安装的 Skill 集合，源码和维护文档由对应版本的源码归档提供；两种安装方式均保留，不改变 Skill 能力和 V1 / VNext 边界。
...
9. Planning 的活跃入口不会长期累积已完成任务正文，单任务详情可按需加载并在完成后进入里程碑归档
10. 当前功能复杂或分散时有唯一、可验证、可统计的权威文档；简单项目和已有同类功能说明文档不被强制拆分
11. Codebase Audit 能跨会话恢复，且业务源码或 Context 漂移后不会继续发布旧 confirmed 结论
12. Codebase Audit 的 Report 已从文档中心可达，或明确记录待执行的 Docs Refresh handoff
