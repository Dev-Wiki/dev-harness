...
- `<docs-root>/plan/tasks/<Task-ID>.md`：执行层，每个活跃任务独立保存背景、目标、权威上下文、代码与测试入口、不变量、影响文件、建议步骤、验收、决策、停止条件和验证证据；
...
4. 使用稳定状态：`📋 规划中`、`🟢 待执行`、`🚧 开发中`、`✅ 已完成`、`📋 远期`；只有执行包完整的任务才能进入 `🟢 待执行`。
5. 未经实现和验证证据，不得从计划或聊天推断任务已经完成。

### 4.4 领取并开发一个任务

Planning 只负责生成和维护可交接的工作顺序、`🟢 待执行`状态与任务执行包，不负责选择执行方式、领取任务或编排开发。用户可以在新对话中明确指定 Task ID；需要自动拾取、恢复或并发控制时，由独立执行 Skill、Agent 工作流或外部编排器负责，并以 Dashboard 当前工作顺序、依赖和阻塞为输入，不根据优先级建立第二套顺序。

执行方解析出唯一 Task ID 后，只在 Dashboard 将任务状态从 `🟢 待执行` 改为 `🚧 开发中`，然后加载对应任务文件、其中列出的必要上下文和实际相关代码，不读取全部活跃或已完成任务正文。领取或恢复任务前记录临时规划快照，后续若 Dashboard、任务路径或 `HEAD` 出现本次工作之外的变化，先重新读取并核对，不能沿用旧选择。
...
提交后，执行方重新读取 Dashboard，按用户指定或独立执行工作流的选择结果处理下一个无阻塞任务，并重复开发闭环。一个里程碑的计划内任务全部完成后，再执行 fresh `harness:full` 和可用的 QA，汇总并收口里程碑归档：
