...
v1.11.5 adds batched Audit state updates, compact document rendering from persisted evidence, and completion checks for the current rendered revision. It preserves drift and cross-module review gates, supports existing hand-authored audit outputs, and stays within the V1 boundary. See the [Audit runtime interface](../codebase-audit/references/runtime-interface.md) for inputs and completion order.
